(function () {
  ...
  function frame(ts){var d=Math.min(100,ts-lastFrame||80);lastFrame=ts;var n=1-Math.pow(.985,d);for(var p=0;p<4;p++)for(var c=0;c<3;c++)current[p][c]=mix(current[p][c],target[p][c],n);userEnergy=mix(userEnergy,targetUser,.26);assistantEnergy=mix(assistantEnergy,targetAssistant,.21);urgency=mix(urgency,targetUrgency,.08);if(ts-lastTexture>900){refreshTextures();lastTexture=ts}
    var t=ts*.00012,motion=1-urgency*.58;ctx.globalCompositeOperation='source-over';ctx.fillStyle='#090a09';ctx.fillRect(0,0,w,h);ctx.globalCompositeOperation='screen';var s=(music?1:.47)*(1-urgency*.25)
  ...
  refreshTextures();window.HerThingVisuals={setArtwork:setArtwork,setVoice:setVoice,setUrgency:setUrgency,setPalette:setPalette};requestAnimationFrame(frame)
})()
